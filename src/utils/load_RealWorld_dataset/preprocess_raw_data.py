...
from utils.load_RealWorld_dataset.preprocessing import *
...
def preprocess_raw_data(DATA_DIR, SUBJECTS, TRAIN_SUBJECTS_ID,
    ...
    all_data = np.load(os.path.join(DATA_DIR,'Clean_Real_World.npy'), allow_pickle=True)
    ...
    for (id_F,Frag) in enumerate(curFragList):
        if Frag.shape[0] < window_size:
            continue
        for pos_id in range(7):
            cur_pos_frag       = Frag[:, (3+pos_id*9) : (3+6+pos_id*9)]
